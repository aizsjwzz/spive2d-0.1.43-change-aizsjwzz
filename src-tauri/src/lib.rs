...
async fn handle_dropped_path(
    ...
    if path_obj.is_dir() {
        ...
        if !unity_bundles.is_empty() {
            ...
            let result = get_subdir_files(temp_path, merge_sequential, sort_mode.clone(), app_handle.clone());
            ...
            get_subdir_files(path, merge_sequential, sort_mode.clone(), app_handle)
        }
            ...
            .map(|e| e.to_lowercase())
        {
            Some(ext) if ext == "zip" || ext == "7z" => {
                ...
                let result = get_subdir_files(final_path, merge_sequential, sort_mode.clone(), app_handle.clone());
                let state = app_handle.state::<AppState>();
                let mut temp_dirs = state.temp_dirs.lock().unwrap();
                temp_dirs.push(temp_dir);
                if temp_dirs.len() > 2 {
...
async fn handle_dropped_paths(
    ...
    app_handle.emit("progress", true).unwrap();
    if paths.len() == 1 {
        let result = handle_dropped_path(paths[0].clone(), merge_sequential, sort_mode.clone(), skip_unity, app_handle.clone()).await;
        let _ = app_handle.emit("progress", false);
        return result;
    }
    let spive_temp_root = std::env::temp_dir().join("spive2d");
...
fn get_subdir_files(
    ...
    match process_directory_with_subdirs(root_path, root_path, merge_sequential, &sort_mode) {
        Ok(subdir_map) => {
...
fn process_directory_with_subdirs(
    ...
    let current_file_groups = process_files(dir_path, base_path, merge_sequential, sort_mode)?;
    ...
    for entry in fs::read_dir(dir_path).map_err(|e| e.to_string())? {
        ...
        if entry_path.is_dir() {
            ...
            let subdir_file_groups = process_directory(&entry_path, base_path, merge_sequential, sort_mode)?;
...
fn process_files(dir_path: &Path, base_path: &Path, merge_sequential: bool, sort_mode: &str) -> Result<Vec<SceneData>, String> {
    println!(
    "[SORT DEBUG] dir={:?}, sort_mode={}",
    dir_path,
    sort_mode
    );
    let mut file_groups = Vec::new();
     // 读取 spive2d_config.json
    let config_path = dir_path.join("spive2d_config.json");
        ...
        match fs::read_to_string(&config_path) {
            Ok(config_text) => {
                match serde_json::from_str::<serde_json::Value>(&config_text) {
                    Ok(config) => {
                        if let Some(draworder) = config.get("draworder").and_then(|v| v.as_array()) {
                            println!(
                                "[DRAWORDER DEBUG] config: {:?}",
                                draworder
                                    .iter()
                            ...
                            println!("[DRAWORDER DEBUG] draworder not found");
                        }
                    }
                    Err(e) => {
                        println!("[DRAWORDER DEBUG] JSON parse error: {}", e);
                    }
                }
            }
            Err(e) => {
                println!("[DRAWORDER DEBUG] read config error: {}", e);
            }
        }
    } else {
        println!("[DRAWORDER DEBUG] config not found: {:?}", config_path);
    }
    ...
    if sort_mode == "desc" {
    file_groups.sort_unstable_by(|a, b| compare_natural(&b.name, &a.name));
} else if sort_mode == "config" {
    // 配置文件排序
    let config_path = dir_path.join("spive2d_config.json");
    ...
    if let Ok(config_text) = fs::read_to_string(&config_path) {
        if let Ok(config) = serde_json::from_str::<serde_json::Value>(&config_text) {
            if let Some(draworder) = config.get("draworder").and_then(|v| v.as_array()) {
                let order: Vec<&str> = draworder
                    .iter()
                    .filter_map(|v| v.as_str())
                    .collect();

                file_groups.sort_unstable_by(|a, b| {
                    let index_a = order.iter().position(|name| *name == a.name);
                    let index_b = order.iter().position(|name| *name == b.name);

                    match (index_a, index_b) {
                        (Some(ia), Some(ib)) => ib.cmp(&ia),
                        (Some(_), None) => std::cmp::Ordering::Less,
                        (None, Some(_)) => std::cmp::Ordering::Greater,
                        (None, None) => compare_natural(&a.name, &b.name),
                    }
                });
            }
        }
    ...
    println!(
    "[SORT DEBUG] order: {:?}",
    file_groups.iter().map(|g| &g.name).collect::<Vec<_>>()
    );
...
fn process_directory(dir_path: &Path, base_path: &Path, merge_sequential: bool, sort_mode: &str) -> Result<Vec<SceneData>, String> {
